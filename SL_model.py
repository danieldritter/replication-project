...
import process
from constants import SEASON, UNIT_POWER
...
class SL_model(Model):
    ...
    def call(self, state_inputs, order_inputs, power_season):
        embedding = self.encoder.call(state_inputs, order_inputs, power_season)
...
if __name__ == "__main__":
    ...
    for i in range(len(state_inputs)):
        ...
        powers_seasons = [SEASON[season_names[i][j][0]] + UNIT_POWER["AUSTRIA"] for j in range(len(season_names[i]))]
